...
function resolveWsUrl(input?: string): string {
  ...
  const defaultUrl = `${scheme}://${location.host}/ws/municipalities`;
  if (!input) return defaultUrl;
  const trimmed = input.trim();
  if (/^wss?:\/\//i.test(trimmed)) {
    return /\/ws\/municipalities(\/?$)/.test(trimmed)
      ? trimmed
      : `${trimmed.replace(/\/$/, '')}/ws/municipalities`;
  }
  if (/^https?:\/\//i.test(trimmed)) {
    const url = new URL(trimmed);
    const wsBase = `${url.protocol === 'https:' ? 'wss' : 'ws'}://${url.host}${url.pathname}`.replace(/\/$/, '');
    return /\/ws\/municipalities(\/?$)/.test(wsBase) ? wsBase : `${wsBase}/ws/municipalities`;
  }
